...
const Cat = () => {
  ...
  const [statusChange, setStatusChange] = useState(localStorage.getItem("CODE") ? localStorage.getItem("CODE") : localStorage.setItem("CODE", "418"))
  const [delay, setDelay] = useState('');
  const [status, setStatus] = useState('');

0
  useEffect( () => {
      const colorInterval = setInterval( () => setColorIdx( prev => (prev + 1) % COLORS.length), delayChange)
